...
export function RecallNotices({ showAll = false }: { showAll?: boolean }) {
  const { pantry, loading: pantryLoading, error: pantryError } = useKitchen();
  ...
  const notices = (feed?.items || []).map((item) => ({
    ...item, matches: potentialPantryMatches(item, pantry),
  })).filter((item) => showAll || item.matches.length > 0);
  return (
    <View style={{ gap: 14 }}>
      <Text style={s.title}>{showAll ? "All current recalls" : "Recall notices"}</Text>
      <Text style={s.body}>
        {showAll
      ...
      <Text style={s.muted}>
        These are not confirmed matches to your pantry. Open
        the notice to check the brand, package, lot, and instructions. This feed
        does not cover every recall.
      </Text>
      ...
      {!showAll && pantryLoading && <Text style={s.muted}>Loading pantry items…</Text>}
      {!showAll && !!pantryError && (
      ...
      {notices.map((item) => (
        <View key={item.id} style={s.card}>
          ...
          {!showAll && (
            <Text style={s.body}>Potential match: {item.matches.map((match) => match.name).join(", ")}</Text>
          )}
      ...
      {feed && !feed.stale && !loading && !error && !notices.length &&
        (showAll || (!pantryLoading && !pantryError)) && (
        <Text style={s.body}>
          {showAll ? "No notices in the current feed." : pantry.some((item) => item.quantity > 0)
      ...
      {!showAll && <Button secondary title="Show all" onPress={() => router.push("/recalls")} />}
      {showAll && <View style={s.card}>
